...
function simulate (sensorSimulation) {
  setTimeout(function () {
  ...
  setTimeout(function () {
    ...
    var inter = setInterval(function () {
      notifier.emit('event', createEvent('rssiUpdated', randomBetween(-99, -20), '713724cf9f8f4b23827c623c7ddbe0fb'));
    }, 3000);
    ...
    setTimeout(function () {
      ...
      clearTimeout(inter);
    }, sensorSimulation.disconnectAfter);

  }, sensorSimulation.connectAfter);
}

module.exports = function (sensorSimulation) {
  if (!inited) {
    logger = logging.getLogger('sensortag');

    if (sensorSimulation && sensorSimulation.enable) {
      simulate(sensorSimulation);
    } else {
      SensorTag.discover(found);
    }
