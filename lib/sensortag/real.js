...
function temperature () {
  logger.info('subscribe for IrTemperature changes');

  sensorTag.on('irTemperatureChange', function (objectTemperature, ambientTemperature) {
    notifier.emit('event', createEvent('temperatureChanged', {
      objectTemperature: objectTemperature,
      ambientTemperature: ambientTemperature
    }, sensorTag.uuid));
  });

  sensorTag.notifyIrTemperature(function (err) {
    if (err) {
      logger.error('failed to subscribe for IrTemperature changes', err);
      return;
    }
    logger.info('IrTemperature changes subscribed');
  });

  logger.info('enable IrTemperature');
  sensorTag.enableIrTemperature(function (err) {
    if (err) {
      logger.error('failed to enable IrTemperature', err);
      return;
    }
    logger.info('IrTemperature enabled');
  });
}
