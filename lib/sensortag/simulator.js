...
var rssiUpdatedInterval = null,
  temperatureChangedInterval = null,
...
function simulateConnect() {
  ...
  temperatureChangedInterval = setInterval(function () {
    notifier.emit('event', createEvent('temperatureChanged', {
      objectTemperature: randomBetween(5, 40),
      ambientTemperature: randomBetween(15, 30)
    }, uuid));
  }, 1000);
}
...
function simulateDisconnect() {
  ...
  if (rssiUpdatedInterval) {
    clearTimeout(rssiUpdatedInterval);
  }

  if (temperatureChangedInterval) {
    clearTimeout(temperatureChangedInterval);
  }
